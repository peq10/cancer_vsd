...
figsave = Path(Path.home(),'Dropbox/Papers/cancer/v1/TTX_washin')
if not figsave.is_dir():
    figsave.mkdir(parents = True)

df = df[(df.use == 'y') & (df.expt == 'TTX_washin')]
...
ax.plot(wash_loc,mean_tcs.shape[0] - 0.5,marker = (3,0,180),markersize = 15, mfc = 'r',mec = 'r')
ax.plot(wash_loc*np.ones(2),[-0.1,mean_tcs.shape[0] - 0.5],'r',linewidth = 2)
pf.plot_scalebar(ax, 500, 2, 100, 0.5,thickness = 4)
plt.axis('off')
fig.savefig(Path(figsave,'TTX_washin.png'),bbox_inches = 'tight',dpi = 300)