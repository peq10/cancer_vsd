...
def get_LED_calibration(top_dir, save_dir):
    ...
    fnames = [
        "/home/peter/data/Firefly/cancer/20201113/LED_calibration.smr",
    ...
    for ii, fname in enumerate(fnames):
        ...
        ax.plot(
            res_blue[:, 1],
            res_blue[:, 1] * fit_blue.slope + fit_blue.intercept,
            "k",
            linewidth=3,
        )
        ...
        ax.plot(
            res_green[:, 1],
            res_green[:, 1] * fit_green.slope + fit_green.intercept,
            "k",
            linewidth=3,
        )
        ...
        np.save(
            Path(save_dir, f"LED_calibration_{dates[ii]}_blue.npy"),
            [fit_blue.slope, fit_blue.intercept],
        )
        np.save(
            Path(save_dir, f"LED_calibration_{dates[ii]}_green.npy"),
            [fit_green.slope, fit_green.intercept],
        )
