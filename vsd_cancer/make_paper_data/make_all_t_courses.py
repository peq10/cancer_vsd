...
def make_all_tc(
    ...
    for idx, data in enumerate(df.itertuples()):
        ...
        if HPC_num is None:
            try:
                with Parallel(n_jobs=njobs) as parallel:
                    ...
                    eroded_tc = parallel(
                        delayed(canf.t_course_from_roi)(stack, mask)
                        for mask in eroded_masks
                    )
                    median_tc = parallel(
                        delayed(canf.median_t_course_from_roi)(stack, mask)
                        for mask in masks
                    )
                    eroded_median_tc = parallel(
                        delayed(canf.median_t_course_from_roi)(stack, mask)
                        for mask in eroded_masks
                    )
                    surround_tc = parallel(
                        delayed(canf.t_course_from_roi)(stack, mask)
                        for mask in surround_masks
                    )
                    median_surround_tc = parallel(
                        delayed(canf.median_t_course_from_roi)(stack, mask)
                        for mask in surround_masks
                    )
                    std = parallel(
                        delayed(canf.std_t_course_from_roi)(stack, mask, True)
                        for mask in masks
                    )
                    surround_std = parallel(
                        delayed(canf.std_t_course_from_roi)(stack, mask, True)
                        for mask in masks
                    )
                ...
                eroded_tc = [
                    canf.t_course_from_roi(stack, mask) for mask in eroded_masks
                ]
                median_tc = [
                    canf.median_t_course_from_roi(stack, mask) for mask in masks
                ]
                eroded_median_tc = [
                    canf.median_t_course_from_roi(stack, mask) for mask in eroded_masks
                ]
                surround_tc = [
                    canf.t_course_from_roi(stack, mask) for mask in surround_masks
                ]
                median_surround_tc = [
                    canf.median_t_course_from_roi(stack, mask)
                    for mask in surround_masks
                ]
                std = [canf.std_t_course_from_roi(stack, mask, True) for mask in masks]
                surround_std = [
                    canf.std_t_course_from_roi(stack, mask, True) for mask in masks
                ]
        else:
            tc = [canf.t_course_from_roi(stack, mask) for mask in masks]
            ...
            median_surround_tc = [
                canf.median_t_course_from_roi(stack, mask) for mask in surround_masks
            ]
            std = [canf.std_t_course_from_roi(stack, mask, True) for mask in masks]
            surround_std = [
                canf.std_t_course_from_roi(stack, mask, True) for mask in masks
        ...
        np.save(
            Path(trial_save, f"{trial_string}_all_eroded_median_tcs.npy"),
            eroded_median_tc,
        )
        np.save(Path(trial_save, f"{trial_string}_all_surround_tcs.npy"), surround_tc)
        np.save(
            Path(trial_save, f"{trial_string}_all_median_surround_tcs.npy"),
            median_surround_tc,
        )
