...
def make_figures(figure_dir,filetype = '.png'):
    ...
    fig.savefig(Path(figsave,f'patch_figure{filetype}'),dpi = 300,bbox_inches = 'tight',transparent = True)
    ...
    fig.savefig(Path(figsave,f'patch_figure_red_dots{filetype}'),dpi = 300,bbox_inches = 'tight',transparent = True)
    ...
    fig.savefig(Path(figsave,f'patch_figure_old_violin{filetype}'),dpi = 300,bbox_inches = 'tight',transparent = True)
    ...
    with open(datafile,'w') as f:
        ...
        f.write(f'Ratio sensitivity mean = {sens[:,-1].mean()} sem. {sens[:,-1].std()/np.sqrt(sens.shape[0])}\n')
