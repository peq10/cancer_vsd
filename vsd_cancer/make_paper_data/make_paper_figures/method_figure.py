...
def make_spectral_shift_trace(
    figsave, save_dir, filetype, delta=20, exc1=405, exc2=530
):
    ...
    fig.savefig(
        Path(figsave, f"regular_spectrum{filetype}"),
        bbox_inches="tight",
        transparent=True,
    )
    # pf.make_square_plot(ax)
...
def make_example_trace(initial_df, save_dir, figsave, filetype):
    ...
    plt.axis("off")
    pf.plot_scalebar(
        ax, 0, 1.5 * (tc_filt.min() - 1) * 100 + offset, 50, 1, thickness=3
    )

    fig.savefig(
        Path(figsave, f"example_tc{filetype}"),
        bbox_inches="tight",
        dpi=300,
        transparent=True,
    )
