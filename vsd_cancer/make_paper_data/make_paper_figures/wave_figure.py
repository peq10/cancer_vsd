...
def plot_correlation_analysis(top_dir, save_dir, figsave, filetype):
    ...
    all_trains = np.load(
        Path(save_dir, "correlation", "all_spike_trains.npy"), allow_pickle=True
    )
    ...
    means = [
        np.mean(corrf.get_all_pairwise(all_trains, binsize)) for binsize in binsizes
    ]
    ...
    fig.savefig(
        Path(figsave, f"Correlation_binchange{filetype}"),
        bbox_inches="tight",
        dpi=300,
        transparent=True,
    )
...
def plot_example_synchrony(top_dir, save_dir, figsave, filetype, redo_boot=False):
    ...
    fig.savefig(
        Path(figsave, f"example_traces{filetype}"),
        bbox_inches="tight",
        dpi=300,
        transparent=True,
    )
    ...
    fig.savefig(
        Path(figsave, f"example_spike_trains{filetype}"),
        bbox_inches="tight",
        dpi=300,
        transparent=True,
    )
    ...
    if redo_boot:
        ...
        fig.savefig(
            Path(figsave, f"example_null_vs_corr{filetype}"),
            bbox_inches="tight",
            dpi=300,
            transparent=True,
        )
...
def plot_wave(top_dir, save_dir, figsave, filetype):
    ...
    dists = (
        np.sqrt((coms[:, 0] - coms[0, 0]) ** 2 + (coms[:, 1] - coms[0, 1]) ** 2) * 1.04
    )
    ...
    for i in range(num):
        line = ax.plot(
            np.arange(roi_t.shape[-1]) * T,
            (roi_t[i] - 1) * 100 + i * 100 / sep,
            color=cmap(i / num),
        )
        line2 = ax.plot(
            np.arange(roi_t.shape[-1]) * T,
            (roi_t_filt[i] - 1) * 100 + i * 100 / sep,
            color="k",
        )
        colors.append(line[0].get_c())
        ax.text(
    ...
    struct[1, ...] = 1
    over = np.logical_xor(
        ndimage.binary_dilation(over, structure=struct, iterations=2), over
    ).astype(int)
    over = np.sum(over[..., None] * colors[:, None, None, :], 0).astype(np.uint8)
    length = int(100 / 1.04)
    ...
    fig.savefig(
        Path(figsave, "../wave_figure2", f"wave_time_courses{filetype}"),
        bbox_inches="tight",
        dpi=300,
    )
