...
def plot_example_and_tcs(save_dir, figsave, filetype):
    mc_str = (
        "cancer_20210119_slip2_area1_long_acq_corr_long_acq_blue_0.0454_green_0.0671_1"
    )
    ...
    for idx, t in enumerate(tds):
        ...
        pf.label_roi_centroids(
            ax1, masks[:num_traces, ...], colors / 255, fontdict={"fontsize": 8}
        )

        fig.savefig(
            Path(figsave, "examples", f"MCF_TGF_Examples_{idx}{filetype}"),
            bbox_inches="tight",
            dpi=300,
            transparent=True,
        )
...
def plot_compare_mda(
    ...
    fig.savefig(
        Path(figsave, "summary", f"MCF_compare_density_{density}_{key}{filetype}"),
        bbox_inches="tight",
        dpi=300,
        transparent=True,
    )
    ...
    if redo_stats:
        p_mda_mcf, _, f1 = statsf.bootstrap_test(
            ...
            names=["MDA-MB-231", "MCF10A"],
        )
        p_mda_tgf, _, f2 = statsf.bootstrap_test(
            md,
            tg,
            function=function,
            plot=True,
            num_resamplings=num_resamplings,
            names=["MDA-MB-231", "MCF10A + TGF$\\beta$"],
        )
        p_mcf_tgf, _, f3 = statsf.bootstrap_test(
            tg,
            mc,
            function=function,
            plot=True,
            num_resamplings=num_resamplings,
            names=["MCF10A + TGF$\\beta$", "MCF10A"],
        )

        f1.savefig(
            Path(figsave, "bootstrap", f"bootstrap_231_MCF_{key}{filetype}"),
            bbox_inches="tight",
            dpi=300,
            transparent=True,
        )
        f2.savefig(
            Path(figsave, "bootstrap", f"bootstrap_231_tgf_{key}{filetype}"),
            bbox_inches="tight",
            dpi=300,
            transparent=True,
        )
        f3.savefig(
            Path(figsave, "bootstrap", f"bootstrap_tgf_MCF_{key}{filetype}"),
            bbox_inches="tight",
            dpi=300,
            transparent=True,
        )
        ...
        with open(
            Path(figsave, "summary", f"statistical_test_results_{key}.txt"), "w"
        ) as f:
            ...
            for h in histtype:
                ...
                fig.savefig(
                    Path(
                        figsave,
                        "histograms",
                        f"MCF_histograms_{h}_log_{l}_onlyneg_{n}{filetype}",
                    ),
                    bbox_inches="tight",
                    dpi=300,
                    transparent=True,
