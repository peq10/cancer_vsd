...
def make_all_cellfree_tc(df_file,save_dir, redo = True, HPC_num = None):
    ...
    if redo:
        ...
        redo_from = np.load(Path(save_dir,f'{df_file.stem}_redo_from_make_all_cellfree_tc.npy'))
    ...
    for idx,data in enumerate(df.itertuples()):
        ...
        redo_from += 1
        np.save(Path(save_dir,f'{df_file.stem}_redo_from_make_all_cellfree_tc.npy'),redo_from)
