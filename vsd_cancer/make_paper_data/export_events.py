...
def export_events(initial_df, save_dir, thresh_idx, min_ttx_amp=1, amp_threshold=None):
    ...
    for idx, data in enumerate(df.itertuples()):
        ...
        for c_id in cell_ids:
            ...
            row = user_input_df[
                (user_input_df.trial_string == data.trial_string)
                & (user_input_df.cell_id == c_id)
            ]
            if len(row) != 1:
    ...
    for idx, data in enumerate(df.itertuples()):
        ...
        active_cell_use = [
            user_input_df[
                (user_input_df.trial_string == data.trial_string)
                & (user_input_df.cell_id == x)
            ].correct.values[0]
            for x in active_cell_ids
        ]
        ...
        active_cell_ids = [
            x
            for x in active_cell_ids
            if np.max(np.abs(events["event_props"][x][:, 1])) > min_ttx_amp / 100
        ]

        only_pos = [
            x for x in active_cell_ids if np.all(events["event_props"][x][:, 1] > 0)
        ]
        ...
        tot_cells.append(
            events["tc"].shape[0] - np.sum(np.logical_not(active_cell_use))
        )  # have to get rid of exclusions
