...
def include_user_input(initial_df, save_dir, thresh_idx):
    ...
    for idx, data in enumerate(df.itertuples()):
        ...
        new_results["observation_length"] = copy.deepcopy(
            results["observation_length"][thresh_idx]
        )
        ...
        for key in new_events.keys():
            ...
            user_input = np.load(
                Path(trial_save, f"{trial_string}_good_detection_cell_{key}.npy")
            )
            if user_input == False:
        ...
        for cell in to_delete:
            ...
            new_events["user_excluded_event_props"][cell] = copy.deepcopy(
                new_events["event_props"][cell]
            )
            del new_events["event_props"][cell]
            # also need to modify observation length
            new_results["observation_length"][
                cell
            ] = 0  # if user excludes, its for whole video

        new_results["events"] = new_events

        np.save(
            Path(
                trial_save, f"{trial_string}_event_properties_including_user_input.npy"
            ),
            new_results,
        )
