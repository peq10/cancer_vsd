...
def load_all_long(df_file, save_dir, redo=True, HPC_num=None, raise_err=False):
    ...
    for idx, data in enumerate(df.itertuples()):
        ...
        for key in result_dict.keys():
            if key == "ratio_stack":
                np.save(
                    Path(trial_save, f"{trial_string}_{key}.npy"),
                    result_dict[key].astype(np.float32),
                )
            else:
...
def load_all_long_washin(df_file, save_dir, redo=True, HPC_num=None, raise_err=False):
    ...
    df = df[
        (df.use == "y") & ((df.expt == "high_k_washin") | (df.expt == "TTX_washin"))
    ]
    ...
    for idx, data in enumerate(df.itertuples()):
        ...
        for key in result_dict.keys():
            if key == "ratio_stack":
                np.save(
                    Path(trial_save, f"{trial_string}_{key}_washin.npy"),
                    result_dict[key].astype(np.float32),
                )
            else:
                continue
