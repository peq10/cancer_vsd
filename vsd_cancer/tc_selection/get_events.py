...
def get_measure_events(initial_df,save_dir,thresh_range = np.arange(2,4.5,0.5),
    ...
    for idx, data in enumerate(df.itertuples()):
        ...
        for detection_thresh in thresh_range:
            ...
            events2 = canf.get_events_exclude_surround_events(tc,
                                                       std,
                                                       surround_tc,
                                                       surround_std,
                                                       z_score = detection_thresh,
            ...
            event_with_props2 = canf.get_event_properties(events2,use_filt = False) 
            
            all_events.append(event_with_props)
            all_events2.append(event_with_props2)
            all_observation.append(canf.get_observation_length(events))
            all_observation2.append(canf.get_observation_length(events2))
        ...
        if exclude_circle == False:
            ...
            result_dict2 = {'n_cells': tc.shape[0],
                          'events': all_events2,
                          'observation_length': all_observation2,
                          'excluded_circle': excluded_circle,
                          'detection_params': detect_params
                          }
        else:
            ...
            result_dict2 = {'n_cells': tc.shape[0] - len(excluded_circle),
              'events': all_events2,
              'observation_length': all_observation2,
              'excluded_circle': excluded_circle,
              'detection_params': detect_params
        ...
        if not yilin_save:
            ...
            np.save(Path(trial_save,f'{trial_string}_event_properties_new_criteria.npy'),result_dict)
            np.save(Path(trial_save,f'{trial_string}_event_properties_old_criteria.npy'),result_dict2)
            
        else:
            print('Not overwriting previous event props - saving for yilin')
            ypath = Path(trial_save,f'{trial_string}_event_properties_yilin_copy_new_criteria.npy')
            print(ypath.absolute())
            np.save(ypath,result_dict)
            
            ypath2 = Path(trial_save,f'{trial_string}_event_properties_yilin_copy_old_criteria.npy')
            print(ypath.absolute())
            np.save(ypath2,result_dict2)
