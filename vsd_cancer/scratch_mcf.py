...
def plot_average_mda_mcf(mda,mcf,tgf, function = np.mean,num_resamplings = 10**5,scale = 4):
    ...
    p_mda_mcf = statsf.bootstrap_test(mda,mcf,function = function,plot = True,num_resamplings = num_resamplings)
    p_mda_tgf = statsf.bootstrap_test(mda,tgf,function = function,plot = True,num_resamplings = num_resamplings)
    p_mcf_tgf = statsf.bootstrap_test(tgf,mcf,function = function,plot = True,num_resamplings = num_resamplings)
...
def plot_MCF_summary(df,key = 'neg_event_rate', function = np.mean):
    ...
    bins = np.histogram(np.concatenate((md,mc,tg)),bins = 20)[1]
